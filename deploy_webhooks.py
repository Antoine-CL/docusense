...
def create_storage_account():
    ...
    cmd = f"az storage account create --name {STORAGE_ACCOUNT_NAME} --resource-group {RESOURCE_GROUP} --location {LOCATION} --sku Standard_LRS"
    run_command(cmd)
...
def create_app_service_plan():
    ...
    cmd = f"az appservice plan create --name {APP_SERVICE_PLAN} --resource-group {RESOURCE_GROUP} --location {LOCATION} --sku Y1 --is-linux"
    run_command(cmd)
...
def create_function_app():
    ...
    cmd = f"az functionapp create --name {FUNCTION_APP_NAME} --resource-group {RESOURCE_GROUP} --storage-account {STORAGE_ACCOUNT_NAME} --plan {APP_SERVICE_PLAN} --runtime python --runtime-version 3.11 --functions-version 4 --os-type Linux"
    run_command(cmd)
...
python-multipart"""
    ...
    with open("azure_function_webhook.py", "r") as src:
        content = src.read()
...
def deploy_functions():
    ...
    # Deploy to Azure
    cmd = f"az functionapp deployment source config-zip --name {FUNCTION_APP_NAME} --resource-group {RESOURCE_GROUP} --src function_app.zip"
    run_command(cmd)
...
def get_function_urls():
    ...
    # Get function keys
    cmd = f"az functionapp keys list --name {FUNCTION_APP_NAME} --resource-group {RESOURCE_GROUP}"
    keys_result = run_command(cmd)
...
def setup_webhooks(webhook_url):
    ...
    run_command("python3 setup_webhooks.py", capture_output=False)
